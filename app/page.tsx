...
function SectionLabel({ index, title }: { index: string; title: string }) {
  return (
    <div className="mb-8 flex items-baseline gap-4">
      ...
      <h2 className="text-2xl font-medium tracking-tight text-frost sm:text-3xl">
        {title}
      </h2>
    </div>
  );
    ...
    <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-center">
      <div className="flex flex-wrap gap-3">
        <a
          ...
          className="inline-flex items-center gap-2 rounded-full bg-frost px-6 py-3 text-sm font-medium text-ink transition-colors duration-300 hover:bg-flame"
        >
          {label} Telegram
        </a>
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2 rounded-full border border-line px-6 py-3 text-sm font-medium text-frost transition-colors duration-300 hover:border-flame hover:text-flame"
        >
          {label} WhatsApp
        </a>
      </div>
      <a
        ...
        className="font-mono text-sm tracking-wider text-mist transition-colors duration-300 hover:text-frost"
      >
        {PHONE_DISPLAY}
      </a>
    </div>
...
export default function Home() {
  return (
    <main className="relative overflow-hidden">
      {/* ── 01 Hero ─────────────────────────────────────────── */}
      <section className="bg-grid relative flex min-h-screen flex-col justify-center">
        <div
          className="pointer-events-none absolute inset-0"
          style={{
            background:
              "radial-gradient(ellipse 80% 55% at 50% 105%, rgba(76,194,255,0.13), transparent 65%)",
          }}
          aria-hidden="true"
        />
        <VolatilityChart className="pointer-events-none absolute bottom-0 left-0 h-64 w-full sm:h-80" />

        <div className="relative mx-auto w-full max-w-5xl px-6 py-28 sm:px-10">
          <Reveal>
            <p className="mb-6 font-mono text-xs uppercase tracking-[0.35em] text-mist">
              Natural Gas Futures · Systematic Alpha
            </p>
          </Reveal>
          <Reveal delay={100}>
            <h1 className="text-6xl font-semibold tracking-tight text-frost sm:text-8xl">
              <span className="text-flame">Δ</span>RH6Ng1
            </h1>
          </Reveal>
          <Reveal delay={200}>
            <p className="mt-6 max-w-2xl text-xl font-medium text-frost/90 sm:text-2xl">
          ...
          <Reveal delay={400}>
            <div className="mt-10">
              <ContactButtons label="Связаться в" />
            </div>
          </Reveal>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-5xl px-6 py-24 sm:px-10">
          ...
          <Reveal delay={100}>
            <p className="max-w-3xl text-2xl font-light leading-snug tracking-tight text-frost sm:text-3xl">
              Думать вне рынка.{" "}
              <span className="text-flame">Действовать внутри рынка.</span>
            </p>
          </Reveal>
          <Reveal delay={200}>
          ...
          <div className="grid grid-cols-3 gap-3 sm:grid-cols-6 sm:gap-4">
            {TIMEFRAMES.map((tf, i) => (
              <Reveal key={tf} delay={i * 60}>
                <div className="group flex aspect-square items-center justify-center rounded-lg border border-line bg-panel transition-colors duration-300 hover:border-flame-dim">
          ...
          <div className="grid gap-3 sm:grid-cols-2 sm:gap-4">
            {RISK_MATH.map((item, i) => (
              <Reveal key={item.title} delay={(i % 2) * 80}>
                <div className="h-full rounded-lg border border-line bg-ink p-6 transition-colors duration-300 hover:border-flame-dim">
          ...
          <div className="grid gap-3 sm:grid-cols-2 sm:gap-4">
            <Reveal>
              <div className="rounded-lg border border-line bg-panel p-8">
                <p className="font-mono text-5xl font-medium tracking-tight text-flame sm:text-6xl">
                  ≤ 600
                </p>
            ...
            <Reveal delay={100}>
              <div className="rounded-lg border border-line bg-panel p-8">
                <p className="font-mono text-5xl font-medium tracking-tight text-frost sm:text-6xl">
                  ~ 120
                </p>
                <p className="mt-3 text-sm leading-relaxed text-mist">
            ...
            {[
              ...
              <Reveal key={item.term} delay={i * 80}>
                <div className="h-full rounded-lg border border-line bg-panel p-6">
                  <p className="font-mono text-lg font-medium text-flame">
                    {item.term}
                  </p>
                  <p className="mt-3 text-sm leading-relaxed text-mist">
                    {item.text}
      ...
      <section className="relative border-t border-line">
        <div
          ...
          style={{
            background:
              "radial-gradient(ellipse 70% 60% at 50% 0%, rgba(76,194,255,0.08), transparent 65%)",
          }}
          aria-hidden="true"
        />
        <div className="relative mx-auto max-w-5xl px-6 py-28 sm:px-10">
          <Reveal>
            <h2 className="text-4xl font-semibold tracking-tight text-frost sm:text-5xl">
              Обсудить стратегию
            </h2>
          </Reveal>
          <Reveal delay={100}>
          ...
          <Reveal delay={200}>
            <div className="mt-10">
              <ContactButtons label="Написать в" />
            </div>
          </Reveal>
        </div>
      </section>
      ...
      <footer className="border-t border-line">
        <div className="mx-auto max-w-5xl px-6 py-12 sm:px-10">
          <p className="max-w-3xl text-xs leading-relaxed text-mist/70">
